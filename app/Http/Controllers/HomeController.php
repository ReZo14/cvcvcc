...
class HomeController extends Controller
{
    ...
    public function fetchMails(Request $request) {
        ...
        if ($request->has('new') || \Request::is('api/*')) {
    ...
    protected function deleteOldMails() {
        if (env('DELETE_AFTER_VALUE', 0) || env('DELETE_AFTER_DAYS', 0)) {
            ...
            } else if(env('DELETE_AFTER_KEY', 'h') == "h") {
                $messages = $inbox->query()->before(now()->subHours(env('DELETE_AFTER_VALUE')))->limit(5, 1)->get();
            } else if(env('DELETE_AFTER_KEY', 'd') == "m") {
                $messages = $inbox->query()->before(now()->subMinutes(env('DELETE_AFTER_VALUE')))->limit(5, 1)->get();
            } else if(env('DELETE_AFTER_KEY', 'd') == "s") {
                $messages = $inbox->query()->before(now()->subSeconds(env('DELETE_AFTER_VALUE')))->limit(5, 1)->get();
            }
            foreach ($messages as $message) {
                $uid = $message->getUid();
